...
#ifndef _i2c_xcore_ai_
#define _i2c_xcore_ai_
...
*  \param config          Structure containing the ports and bus speed   
*
...
void i2c_master_send_stop_bit(i2c_master_config_t *config);
  ...
  inline uint8_t i2c_master_read_reg(uint8_t device_addr, uint8_t reg,
    ...
    size_t n;
    i2c_res_t res;
    res = i2c_master_write(device_addr, a_reg, 1, &n, 0, config);
    if (n != 1) {
      *result = I2C_REGOP_DEVICE_NACK;
      i2c_master_send_stop_bit(config);
      return 0;
    }
    res = i2c_master_read(device_addr, data, 1, 1, config);
    if (res == I2C_ACK) {
                                    ...
                                    i2c_master_config_t *config)
  {
    ...
    size_t n;
    i2c_master_write(device_addr, a_data, 2, &n, 1, config);
    if (n == 0) {
